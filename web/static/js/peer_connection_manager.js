...
export default class PeerConnectionManager {
  ...
  signallingChannel: any;
  peers: Array<Peer>;

  onPeerAdded: PeerFun;
  onPeerRemoved: PeerFun;

  ///
  // Receives id of current user, 
  // a phoenix channel that will be used for signalling
  // and an array of peer objects: 
  constructor(currentUserId: string, signallingChannel: any, peers: Array<Peer>, onPeerAdded: PeerFun, onPeerRemoved: PeerFun) {
  ...
  onOfferReceived({to, from, sdp}: Object) {
  ...
  onIceCandidateReceived({to, from, iceCandidate}: any) {
    // Handle only events that are intended for us
  ...
  createPeer(peerId: string) {
    ...
    let peer: Peer = {id: peerId, connection: conn, dataChannel: null};
  ...
  onLocalDescCreated(peerId: string, desc: any, connection: RTCPeerConnection) {
    connection
